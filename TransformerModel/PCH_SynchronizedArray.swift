//
//  PCH_SynchronizedArray.swift
//  TransformerModel
//
//  Created by Peter Huber on 2017-11-02.
//  Copyright © 2017 Peter Huber. All rights reserved.
//

import Foundation

// This class comes from http://basememara.com/creating-thread-safe-arrays-in-swift/ and is copied in its entirety from that site. I needed a thread-safe array and this did the trick, so for now I am trusting that the code is correct.

/// A thread-safe array.
public class SynchronizedArray<Element> {
    fileprivate let queue = DispatchQueue(label: "com.huberistech.SynchronizedArray", attributes: .concurrent)
...
public extension SynchronizedArray
{
    ...
    {
        // like the Apple array version, the array must not be empty
        if self.array.isEmpty
        {
            ALog("Array cannot be empty!")
        }
    ...
    // This will return a non-safe Apple array representation of the array. While the routine itself is threadsafe, be aware that the underlying thread-safe array may be changed immediately after returning the copy.
